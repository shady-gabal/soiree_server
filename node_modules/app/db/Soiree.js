...
soireeSchema.statics.createSoireeWithType = function(soireeType, successCallback, errorCallback, options) {
	...
	else if (soireeType === 'Soiree Date'){
		this.createSoireeDate(successCallback, errorCallback);
	}
...
soireeSchema.statics.createSoireeDate = function(successCallback, errorCallback, options) {
	//var date = new Date(todaysDate.getTime() + (7 * 24 * 60 * 60 * 1000));
	if (!options) options = {};

	var Soiree = this;

	var soireeType = "Soiree Date";
	Soiree.determineDateForSoireeType(soireeType, function(date){
		if (!date) return errorCallback(ErrorCodes.NoAvailableDate);
...
soireeSchema.statics.findSoirees = function(req, user, successCallback, errorCallback){
	if (!user){ return errorCallback(ErrorCodes.InvaludInput); }
...
function filterOutSoireesForUser(user, soirees, callback){
	...
	for (var i = 0; i < soirees.length; i++){
		...
		shouldShowSoireeForUser(user, soiree, function(){
			ans.push(soiree);
			if (++numReturned === soirees.length){ return callback(ans); }
