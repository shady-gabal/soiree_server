...
soireeSchema.statics.createSoireeWithTypeForTests = function (soireeType, successCallback, errorCallback, options) {
	...
	if (soireeType === 'dinner') {
		...
		var MovieSoiree = require('app/db/MovieSoiree.js');
		MovieSoiree.createMovieSoiree( {movieName : "Test Movie"}, successCallback, errorCallback);
	}
	else if (soireeType === 'test') {
		this.createTest(successCallback, errorCallback);
	}
...
soireeSchema.methods.addNewReservation = function(reservation){
	console.log("in addNewReservation for user " + reservation._user.fullName ? reservation._user.fullName : reservation._user);

	if(!reservation.charged){
		ArrayHelper.pushOnlyOncePopulated(this, "_unchargedReservations", reservation);
		ArrayHelper.pushOnlyOncePopulated(this, "_usersUncharged", reservation._user);
	}
	else{
		ArrayHelper.pushOnlyOncePopulated(this, "_chargedReservations", reservation);
		ArrayHelper.pushOnlyOncePopulated(this, "_usersAttending", reservation._user);
	}

	return this.save();
};

soireeSchema.methods.chargedUnchargedReservation = function(reservation){
	ArrayHelper.removeObjectPopulated(this, "_unchargedReservations", reservation);
	ArrayHelper.removeObjectPopulated(this, "_usersUncharged", reservation._user);
...
soireeSchema.pre("save", function (next) {
	...
	if (!this.title){
		this.title = capitalize(this.soireeType);
	}
	next();
});
