...
soireeSchema.statics.findNextSoiree = function(user, idsToIgnore, successCallback, errorCallback){
	...
	this.find({cancelled: false, ended: false, started: false, college: college, _usersAttending : {"$ne" : user._id}, _usersUncharged : {"$ne" : user._id}, soireeId : {"$nin" : idsToIgnore}, date: {"$gte" : today}}).sort({"date" : 1}).limit(1).exec(function(err, soirees){
		console.log(soirees);
...
soireeSchema.statics.setSoireeDefaults = function(soiree, successCallback, errorCallback, options){
	...
	if (soiree.date && soiree.college) {
		...
		if (soiree.soireeType != "TEST"){
			console.log("increasing count of [" + soiree.college + "][" + soiree.soireeType + "][" + hoursMinsString + "] from " + timeCountsPerCollege[soiree.college][soiree.soireeType][hoursMinsString] );
			timeCountsPerCollege[soiree.college][soiree.soireeType][hoursMinsString]++;
			console.log("to " + timeCountsPerCollege[soiree.college][soiree.soireeType][hoursMinsString]);
		}
	}
...
function determineDateForSoireeType(soireeType, college, callback){
	...
	console.log("determining date for soiree type "  + soireeType + "for college " + college);
