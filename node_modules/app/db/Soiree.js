...
soireeSchema.methods.startIfPossible = function(){
	if (!this.cancelled && !this.ended && !this.started) {
		...
		var todaysDate = new Date();
		var numMinsPassed = (todaysDate - this.date)/(60 * 1000);
		...
		if (canStart){
			if (!timeWaitingUp){
				...
				if (this._host._usersJoined.length >= this.numUsersMin) {
					this.start();
				}
				//else keep waiting for new users to join or time limit to be up
			}
			else{
				if (this._host._usersJoined.length >= 2) {
					this.start();
				}
				else {
			...
			//if everyone is here
			if (this._host._usersJoined.length >= this._usersAttending.length) {
				this.start();
			}
		}
	}
