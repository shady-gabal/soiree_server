...
soireeSchema.statics.findNextSoiree = function(user, idsToIgnore, successCallback, errorCallback){
	...
	this.find({college: college, _usersAttending : {"$ne" : user._id}, _usersUncharged : {"$ne" : user._id}, soireeId : {"$nin" : idsToIgnore}, date: {"$gte" : today}}).sort({"date" : 1}).limit(1).exec(function(err, soirees){
		console.log(soirees);
