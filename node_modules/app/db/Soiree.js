...
soireeSchema.methods.jsonObject = function (user) {
	...
	var obj = {
		...
		"subwayLinesNear" : this.subwayLinesNear
...
soireeSchema.statics.findSoirees = function (req, user, successCallback, errorCallback) {
	...
	this.find(constraints).deepPopulate("_business _usersAttending").limit(numSoireesToFetch).exec(function (err, soirees) {
		...
		if (err) {
			...
			console.log('filtering out soirees');
...
function filterOutSoireesForUser(user, soirees, callback) {
	...
	for (var i = 0; i < soirees.length; i++) {
		...
		shouldShowSoireeForUser(user, soiree, function () {
			console.log('filtering completed for soiree ' + soiree.soireeId);
			ans.push(soiree);

			if (++numReturned === soirees.length) {
				console.log('filtering done. returning...');
				return callback(ans);
			}
