...
soireeSchema.methods.remind = function(mins) {
	if (!mins) {
		var todaysDate = new Date();
		var diff = todaysDate - this.date;//in ms
...
soireeSchema.methods.cancel = function(){
	...
	for (var i = 0; i < this._usersUncharged; i++){
		var user = this._usersUncharged[i];
		user.notifyOfSoireeCancellation(this);
	}
	//double check that no users were charged. if so, refund them
	if (this._chargedReservations.length > 0){
		for (var j = 0; j < this._chargedReservations.length; j++){
			var reservation = this._chargedReservations[j];
			reservation.refund();
		}
	}
