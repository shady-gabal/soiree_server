...
soireeSchema.statics.createSoireeWithType = function(soireeType, successCallback, errorCallback, options) {
	...
	else if (soireeType === 'Speed Date'){
		this.createSpeedDate(successCallback, errorCallback);
	}
...
soireeSchema.statics.createSpeedDate = function(successCallback, errorCallback, options) {
	//var date = new Date(todaysDate.getTime() + (7 * 24 * 60 * 60 * 1000));
	if (!options) options = {};

	var Soiree = this;

	var soireeType = "Speed Date";
	Soiree.determineDateForSoireeType(soireeType, function(date){
		if (!date) return errorCallback(ErrorCodes.NoAvailableDate);
