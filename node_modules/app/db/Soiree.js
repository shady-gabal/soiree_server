...
var MIN_TIME_BEFORE_CAN_CREATE_SOIREE = 1.5 * 60 * 60 * 1000;
/* Error Codes */
...
var soireeSchema = new Schema({
		...
		duration : {type: Number, default: AVERAGE_SOIREE_LENGTH},
...
function determineDateForSoireeType(soireeType, college, callback){
	...
	var dateCallback = function(counts){
		...
		var minTime = availableTimes[0];
		var min = counts[minTime];

		for (var i = 1; i < availableTimes.length; i++){
			var curr = availableTimes[i];

			if (counts[curr] < min && minDate <= DateHelper.dateFromTime(curr)){
				min = counts[curr];
				minTime = curr;
			}
		}
...
soireeSchema.statics.createLunch = function(college, successCallback, errorCallback, options) {
	...
	determineDateForSoireeType(soireeType, college, function(date){
		if (!date) return errorCallback(ErrorCodes.Error);

		var hoursMinsString = DateHelper.timeStringFromDate(date);
		timeCountsPerCollege[college][soireeType][hoursMinsString]++;
