...
var soireeSchema = new Schema({
		...
		//scheduledCheckForCancellationTimeIdentifier : {type: String},
...
soireeSchema.statics.findNextSoiree = function(user, idsToIgnore, successCallback, errorCallback){
	...
	this.find({cancelled: false, ended: false, started: false,  _usersAttending : {"$ne" : user._id}, _usersUncharged : {"$ne" : user._id}, soireeId : {"$nin" : idsToIgnore}, date: {"$gte" : today}}).sort({"date" : 1}).limit(1).exec(function(err, soirees){
		console.log(soirees);
...
soireeSchema.statics.setSoireeDefaults = function(soiree, successCallback, errorCallback, options){
	...
	if (options["business"]){
		...
		Business.nextBusinessToHostSoiree(soiree.soireeType, function(business){
...
soireeSchema.statics.createSoireeWithType = function(soireeType, successCallback, errorCallback, options) {
	if (soireeType === "Lunch"){
		this.createLunch(successCallback, errorCallback);
	}
	else if (soireeType === 'Dinner'){
		this.createDinner(successCallback, errorCallback);
	}
	else if (soireeType === 'Drinks'){
		this.createDrinks(successCallback, errorCallback);
	}
	else if (soireeType === 'Blind Date'){
		this.createBlindDate(successCallback, errorCallback);
	}
...
soireeSchema.statics.createLunch = function(successCallback, errorCallback, options) {
	...
	Soiree.determineDateForSoireeType(soireeType, function(date){
...
soireeSchema.statics.createDinner = function(successCallback, errorCallback, options) {
	if (!options) options = {};
...
soireeSchema.statics.createDrinks = function(successCallback, errorCallback, options) {
	if (!options) options = {};
...
soireeSchema.statics.createBlindDate = function(successCallback, errorCallback, options) {
	//var date = new Date(todaysDate.getTime() + (7 * 24 * 60 * 60 * 1000));
	if (!options) options = {};
...
soireeSchema.statics.findSoirees = function(req, user, successCallback, errorCallback){
	if (!user){
		return errorCallback(ErrorCodes.InvaludInput);
	}

	var constraints = { "cancelled" : false, "started" : false, "ended" : false, _usersAttending : {"$ne" : user._id}, _usersUncharged : {"$ne" : user._id}};
	...
	this.find(constraints).deepPopulate("_business _usersAttending").limit(numSoireesToFetch).exec(function(err, soirees){
		if (err){
			...
			var filteredSoirees = filterOutSoireesForUser(user, soirees);
			successCallback(filteredSoirees);
		}
	});
};

function filterOutSoireesForUser(user, soirees){
	var ans = [];
	...
	for (var i = 0; i < soirees.length; i++){
		...
		for (var j = 0; j < soiree._users.length; j++){
			var currUser = soiree._users[j];
	...
	if (mins > 0) {
		...
		this.deepPopulate("_usersAttending", function(err){
			...
			for (var i = 0; i < soiree._usersAttending.length; i++) {
				...
				//var message = "Hey boo. Don't forget that your " + soiree.soireeType + " " + Globals.SOIREE + " will start in " + mins + " minutes. See you there. xoxo";
...
soireeSchema.methods.cancel = function(){
	if (!this.cancelled && !Globals.soireesCancelling[this.soireeId]) {
		...
		this.deepPopulate("_chargedReservations _usersUncharged _usersAttending _business", function (err, soiree) {
			...
			soiree._business.numSoireesHosted--;
			...
			soiree._business.save(Globals.saveErrorCallback);
...
soireeSchema.methods.chargedForReservations = function(reservations){
	for (var i = 0; i < reservations.length; i++){
		this.chargedForReservation(reservations[i]);
	}
};
...
soireeSchema.methods.chargedForReservation = function(reservation){
	...
	//reservation.deepPopulate("_business _user", function(err, _reservation){
		...
		ArrayHelper.removeObjectPopulated(soiree, "_unchargedReservations", reservation);

		//remove user from list of users that we havent charged yet
		ArrayHelper.removeObjectPopulated(soiree, "_usersUncharged", reservation._user);

		//add reservation to soirees chargedReservations
		ArrayHelper.pushOnlyOncePopulated(soiree, "_chargedReservations", reservation);

		//add user to soirees usersAttending
		ArrayHelper.pushOnlyOncePopulated(soiree, "_usersAttending", reservation._user);

		//add reservation to business's unconfirmedReservations
		ArrayHelper.pushOnlyOncePopulated(reservation._business, "_unconfirmedReservations", reservation);

		soiree.save(Globals.saveErrorCallback);
		reservation._business.save(Globals.saveErrorCallback);
		console.log("finished chargeForReservation()");
	//});
...
soireeSchema.methods.join = function(user, successCallback, errorCallback){
	...
	//if (soiree.college !== user.college){
	//	console.log('college mismatch while trying to join soiree. user college: ' + user.college + ' soiree college: ' + soiree.college);
	//	return errorCallback(ErrorCodes.CollegeMismatch);
	//}
...
soireeSchema.methods.chargeUnchargedUsers = function(){
	...
	this.deepPopulate("_unchargedReservations._user", function(err, _soiree){
		...
		var reservationsCharged = [];
		var numToReturn = _soiree._unchargedReservations.length;
		var numReturned = 0;

		_soiree._unchargedReservations.forEach(function(reservation){
			...
			if (reservation.charged){
				...
				numToReturn--;
				if (numReturned === numToReturn)
					soiree.chargedForReservations(reservationsCharged);
			}
			else {
				reservation.chargeUser(function (user) {
					console.log("Successfully charged " + user.fullName);
					Notification.createSoireeConfirmedNotification(_soiree, user);
					reservationsCharged.push(reservation);

					numReturned++;
					if (numReturned === numToReturn) soiree.chargedForReservations(reservationsCharged);
...
soireeSchema.methods.jsonObject = function (user) {
	...
	var obj = {
		...
		"businessAddress" : this._business.businessAddress,
		"businessCityArea" : this._business.cityArea,
		"coordinates" : LocationHelper.locationFromCoordinates(this.location.coordinates),
