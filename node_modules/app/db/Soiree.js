...
var soireeSchema = new Schema({
		...
		whatYouGet : {type: String, default: "Entrance into the venue and the Soirée"},
		restrictions : [{type: String, enum: Globals.soireeRestrictions}],
...
soireeSchema.methods.jsonObject = function (user) {
	...
	var obj = {
		...
		"title" : this.title,
		"category" : Globals.soireeCategoryDisplayNames[this.category] ? Globals.soireeCategoryDisplayNames[this.category] : capitalizeAndClean(this.category),
		"categoryIdentifier" : this.category ? this.category : "",
	...
	if (user) {
		...
		if (obj.userAlreadyJoined){
			...
			obj.coordinates = LocationHelper.locationFromCoordinatesArray(this.location.coordinates);
		}
	}
	return obj;
};
...
soireeSchema.statics.setSoireeDefaults = function (soiree, successCallback, errorCallback, options) {
	...
	var restrictions = (soiree.restrictions && soiree.restrictions.length > 0) ? soiree.restrictions : [];
	...
	var defaultRestrictions = Globals.defaultRestrictionsPerSoireeType[soiree.soireeType] ? Globals.defaultRestrictionsPerSoireeType[soiree.soireeType] : [];
	restrictions = _.unique(restrictions.concat(defaultRestrictions));

	soiree.initialCharge = initialCharge;
	soiree.numUsersMax = numUsersMax;
	soiree.numUsersMin = numUsersMin;
	soiree.restrictions = restrictions;
...
soireeSchema.methods.dateString = function(){
...
soireeSchema.methods.timeString = function(){
	var date = this.date;
	var todaysDate = new Date();
	var hour = date.getHours();
	var minutes = date.getMinutes();
...
soireeSchema.methods.failedToChargeUnchargedReservationForId = function (_id, reservation) {
	_id = _id._id ? _id._id : _id; //chance that object passed in is actually populated doc. if so get its _id
	var updateQuery;
	var Soiree = this;
	updateQuery = { $pull : {_unchargedReservations : reservation._id, _usersUncharged : reservation._user } };

	this.findOneAndUpdate({_id : _id}, updateQuery, {new : true}, function(err, soiree){
		if (err){
			console.log(err);
		}
	});
...
soireeSchema.methods.checkIfUserPassesRestrictions = function(user){
	for (var i = 0; i < this.restrictions.length; i++){
		var restriction = this.restrictions[i];
		if (restriction === "drinking_age"){
			if(user.age < 21){
				var errorMessage = Globals.soireeRestrictionsErrorMessages[restriction] ? Globals.soireeRestrictionsErrorMessages[restriction] : "Sorry but it looks like you can't join this Soirée at the moment.";
...
soireeSchema.methods.join = function (user, successCallback, errorCallback) {
	...
	if (soiree.restrictions && soiree.restrictions.length > 0){
		var error = soiree.checkIfUserPassesRestrictions(user);
		if (error){
			console.log("restriction failed. error message: " + error);
			return errorCallback(ErrorCodes.SoireeRestrictionsFailed, error);
		}
	}
...
soireeSchema.methods.createDescription = function () {
	...
	if (category === "social_soirees"){
		description = "Social Soirées are themed gatherings with tons of new, interesting people that enable you to make an instant connection with dozens of them all at once! Looking to unwind after work? Bored and have nothing to do? Sign up for a Social Soirée to provide a jolt of excitement to your day! You shouldn't have to wait on your friends to be available in order to make plans and have a good time; from drinks and games to even playing paintball, you’ll always have new friends to make memories with thanks to Social Soirées! (Note: Soirées at any drinking establishment are 21+)";
	}
	else if (category === "movie_soirees"){
		description = "Nobody wants to go to the movies alone. But you’re never really alone – movie theatres are full of strangers sitting together for hours. Soirée is the first app to facilitate like-minded strangers actually watching a movie together. Buying a movie ticket on Soirée links you with a group of moviegoers also looking to watch the same movie AND make new connections. Instead of asking and hearing all your friends say they’re busy, a Movie Soirée lets you watch the movie you want on your own time - and make a whole new group of friends at the same time!";
	}
	else if (category === "soirees_for_singles"){
		description = "Date in the real world again. Soirée also hosts unique member-only dating events meant to provide dynamic new dating experiences. Soirée provides outside the box ways to break the ice, from group blind-dates to doubles speed-dates -- best friends speed-dating alongside each other for a night of inside jokes together. Soirée caters experiences for people of all sexual identities, and proudly partners with many venues well-loved by the New York LGBT Community in order to allow everyone the opportunity to find love the real way – in the real world.";
	}
	else{
		description = "A Soirée is a gathering of dozens of new, interesting people, designed to get you to know dozens of them easily, quickly, and personally. Unlike going to a bar or club, Soirées promote conversation and mingling by having funny, friendly comedians hosting each event, breaking the ice and sparking conversation. After going on a single Soirée, you'll have met more new people than you would going to bars or clubs for a whole month!";
	}
