...
var facebookProvider = Globals.providers[0];
var userPwProvider = Globals.providers[1];
...
var userSchema = new Schema({
		...
		provider: {type: String, enum: Globals.providers},
		...
		interestedIn : [{type: String, enum: Globals.genders}],
		//college: {type: String, enum: Globals.colleges}, /* Colleges */
		...
		numUnseenNotifications : {type: Number, default: 0},
...
userSchema.methods.addNotification = function(notification) {
	...
	ArrayHelper.pushOnlyOncePopulated(this, "_notifications", notification, function(added){
		if (added){
...
userSchema.pre("remove", function(next){
	...
	mongoose.model("CommunityPost").remove({_user : this._id}, function(err){
		if (err) console.log(err);
	});
	mongoose.model("CommunityComment").remove({_user : this._id}, function(err){
		if (err) console.log(err);
	});
	mongoose.model("Notification").remove({_user : this._id}, function(err){
		if (err) console.log(err);
	});
