...
var UserProfileQuestions = [
	...
	"Are you a banana",
	"Which do you like more: igneous rocks or sedimentary rocks"
];
...
userSchema.statics.failedToChargeUnchargedReservationForId = function(_id, reservation) {
	...
	this.findOneAndUpdate({_id : _id}, updateQuery, {new : true}).exec(function(err, user){
		if (err){
			...
			var message = "Your card was declined when we tried to charge it for your soirée reservation, so we had to cancel your reservation :( Update your card and rejoin to reclaim it!"
	...
	jwt.verify(encodedAccessToken, jwtUserSecret, {}, function(err, decoded){
		...
		User.findOne({userId : userId}).exec(function(err, user){
			if (err){
				...
				if (user.email === username || user.facebookUserId === username){
					return done(null, user);
				}
				else return done(new Error("Error Verifying"), null);
			}
		});
	});
};
