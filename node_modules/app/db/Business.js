...
var businessSchema = new Schema({
        businessType : {type: String, enum: Globals.businessTypes, lowercase: true, trim: true},
        ...
        _cancelledSoirees : [{type: ObjectId, ref:"Soiree"}],
        _unconfirmedReservations : [{type: ObjectId, ref:"SoireeReservation"}],
        _confirmedReservations : [{type: ObjectId, ref:"SoireeReservation"}],
        _schedule : [{type : ObjectId, ref : "Event", required : [true, "Business must have a schedule"]}],
        location: {
            type: {type: String},
            coordinates: []
        },
...
businessSchema.statics.addNewReservationForId = function(_id, reservation){
    if (reservation.charged){
        _id = _id._id ? _id._id : _id; //chance that object passed in is actually populated doc. if so get its _id
        var updateQuery;

        updateQuery = { $addToSet : {_unconfirmedReservations : reservation._id} };

        return this.findOneAndUpdate({_id : _id}, updateQuery, {new : true}).exec();
    }
    else return this.findOneAndUpdate({_id : _id}, {}).exec();

};


businessSchema.statics.chargedUnchargedReservationForId = function(_id, reservation){
    _id = _id._id ? _id._id : _id; //chance that object passed in is actually populated doc. if so get its _id
    this.addNewReservationForId(_id, reservation);
};

businessSchema.statics.failedToChargeUnchargedReservationForId = function(_id, reservation){
    _id = _id._id ? _id._id : _id; //chance that object passed in is actually populated doc. if so get its _id
    var updateQuery;
    //shouldnt be in _unconfirmedReservations but just in case
    updateQuery = { $pull : {_unconfirmedReservations : reservation._id} };
...
businessSchema.methods.createdNewSoiree = function(soiree){
    var updateQuery = {$addToSet : { _soirees : soiree._id}};

    return mongoose.model("Business").findOneAndUpdate({_id : this._id}, updateQuery, {new : true}).exec();
};

businessSchema.methods.cancelledSoiree = function(soiree){
    var updateQuery = {$addToSet : { _cancelledSoirees : soiree._id}, $pull : {_soirees : soiree._id}, $inc : {numSoireesHosted : -1} };

    mongoose.model("Business").findOneAndUpdate({_id : this._id}, updateQuery, {new : true}).exec(function(err, business){
        if (err){
            console.log(err);
        }
    });
...
businessSchema.pre('validate', function(next){
    if (!this.businessId){
        IDGeneratorHelper.generateUniqueId("Business", "businessId", this, next, {length: 5});
    }
    else next();
});

businessSchema.pre('remove', function(next){
    mongoose.model("Soiree").find({_business : this._id}).exec(function(err, soirees){
