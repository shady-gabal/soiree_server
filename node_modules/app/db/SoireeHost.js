...
var customSchema = new Schema({
        ...
        cahQuestionsAsked : [{type: String}],
        cahAnswersGiven : [{type: String}]
...
customSchema.methods.sendWelcomeMessagesToUser = function(socket) {
    ...
    var welcomeMessages = createMessages([
        "Welcome to this " + Globals.SOIREE_LOWERCASE, "!," +
        " At " + Globals.SOIREE_LOWERCASE + "s, there is no reason to be shy. Walk around and introduce yourself to your fellow " + Globals.SOIREE_LOWERCASE + "goers - trust us, they're amazing people. We hope you have a great time!",
        "Not sure how to introduce yourself? Talking to someone and the conversation hit a standstill? Tap below to get a fun question to ask to spark it up again.",
        "Also, there'll be two cards of humanity games throughout this " + Globals.SOIREE_LOWERCASE + ". Tap below to join them."
    ]);
...
customSchema.methods.requestingStartCAH = function(socket){
    socket.emit('start cah', []);
};

customSchema.methods.askCAHQuestion = function(socket){
    var rand = Math.random() * CAHQuestions.length;
    while(this.cahQuestionsAsked.indexOf(CAHQuestions[rand]) != -1){
        rand = Math.random() * CAHQuestions.length;
    }
    var question = CAHQuestions[rand];

    socket.emit('cahQuestion', question);
    //this.cahQuestionsAsked.push(question);
};

customSchema.methods.getCAHAnswers = function(socket, num){
    var ansArr = [];
    for (var i = 0; i < num; i++) {
        var rand = Math.random() * CAHAnswers.length;
        while (this.cahAnswersGiven.indexOf(CAHAnswers[rand]) != -1) {
            rand = Math.random() * CAHAnswers.length;
        }
        var ans = CAHAnswers[rand];
        //this.cahAnswersGiven.push(ans);
        ansArr.push(ans);
    }

    socket.emit('cahAnswers', ansArr);
