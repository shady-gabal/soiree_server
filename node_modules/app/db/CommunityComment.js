...
commentSchema.statics.upvoteCommentWithId = function(commentId, user, successCallback, errorCallback) {
    ...
    this.findOneAndUpdate({commentId : commentId}, updateQuery, {new : true}, function(err, post){
        if (err){
            console.log(err);
            return errorCallback(ErrorCodes.MongoError);
        }
...
commentSchema.statics.downvoteCommentWithId = function(commentId, user, successCallback, errorCallback) {
    ...
    this.findOneAndUpdate({commentId : commentId}, updateQuery, {new : true}, function(err, comment){
        if (err){
            console.log(err);
            return errorCallback(ErrorCodes.MongoError);
        }
...
commentSchema.methods.upvoted = function(){
    this.deepPopulate("_user", function(err, post){
        ...
        var user = post._user;
        user.updateSoireeScore(Globals.soireeScoreCommentUpvoteIncrement);
    });
};

commentSchema.methods.downvoted = function(){
    this.deepPopulate("_user", function(err, post){
        if (err){
            console.log(err);
            return;
