...
var ssJobSchema = new Schema({
        _user : {type: ObjectId, ref: "User", autopopulate: true},
...
ssJobSchema.statics.performForCollege = function(college){
    ...
    for (var i = 0; i < Globals.soireeTypes.length; i++){
        ...
        this.find({soireeType: soireeType, done: false}, function(err, jobs){
            if (err){
                ...
                ScheduledSoireeJob.matchScheduledSoirees(soireeType, jobs);
            }
...
ssJobSchema.statics.matchScheduledSoirees = function(soireeType, ssJobs){
    var buckets = {};
    for (var k = 0; k < Globals.scheduledSoireeAvailableTimes.length; k++){
        var timeStr = Globals.scheduledSoireeAvailableTimes[k];
        buckets[timeStr] = [];
    }
...
function generateTimesAvailableArray(ssJob){
    ...
    for (var i = 0; i < Globals.scheduledSoireeAvailableTimes.length; i++){
        var currTime = Globals.scheduledSoireeAvailableTimes[i];
...
ssJobSchema.plugin(autopopulate);
module.exports = mongoose.model('ScheduledSoireeJob', ssJobSchema);