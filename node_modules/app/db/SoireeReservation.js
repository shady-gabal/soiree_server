...
customSchema.methods.makeJsonObject = function(callback){
    this.deepPopulate("_user _business", function(err, reservation){

        var obj = {
            "reservationId" : reservation.reservationId,
            "soireeId" : reservation.soireeId,
            "confirmationCode" : reservation.confirmationCode,
            "confirmed" : reservation.confirmed,
            "charged" : reservation.charged,
            "whatYouGet" : reservation.whatYouGet,
            "userName" : reservation._user.fullName,
            "verificationImageUrl" : reservation._user.verificationImageUrl
        };

        if (reservation._business){
            obj["businessName"] = reservation._business.businessName;
        }
...
customSchema.methods.chargedUnchargedReservation = function () {
    ...
    User.chargedUnchargedReservationForId(this._user, this);
    Business.chargedUnchargedReservationForId(this._business, this);
    Soiree.chargedUnchargedReservationForId(this._soiree, this);

};
//called when an uncharged reservation is attempted to be charged but fails because of a card issue
customSchema.methods.failedToChargeUnchargedReservation = function () {

    User.failedToChargeUnchargedReservationForId(this._user, this);
    Business.failedToChargeUnchargedReservationForId(this._business, this);
    Soiree.failedToChargeUnchargedReservationForId(this._soiree, this);
...
customSchema.statics.createUnchargedSoireeReservation = function(user, soiree, successCallback, errorCallback){
    ...
    var reservation = new this({
        ...
        charged : false,
        amount : soiree.initialCharge,
        whatYouGet : soiree.whatYouGet
    });
    ...
    reservation.save(function(err, _reservation) {
        if (err) {
            ...
            var promises = [
                User.addNewReservationForId(user._id, _reservation),
                Soiree.addNewReservationForId(soiree._id, _reservation),
                Business.addNewReservationForId(business._id, _reservation)
            ];
    ...
    if (!this.charged){
        ...
        this.deepPopulate("_soiree _user", function(err, _reservation){
            CreditCardHelper.chargeForSoiree(_reservation._soiree, _reservation._user, function(charge) {
                ...
                _reservation.failedToChargeUnchargedReservation();
                errorCallback(err);
            });
        });
    }
...
customSchema.methods.confirm = function(successCallback, errorCallback){

    var reservation = this;

    if (!this.confirmed){
        if (!DateHelper.isSameDay(new Date(), this._soiree.date) && (Date.now() - this._soiree.date.getTime() > 12 * 60 * 60 * 1000)){
        ...
        this.save(function(err){
            if (err){
                ...
                reservation.populate("_business _user", function(err){
                    if (err){
                        console.log(err);
                        return errorCallback(ErrorCodes.ErrorSaving);
                    }

                    reservation._business.confirmSoireeReservation(reservation, function(){
                        return successCallback(reservation._user);
                    }, errorCallback);
    ...
    else errorCallback(ErrorCodes.AlreadyExists);
};

customSchema.methods.confirmByConfirmationCode = function(code, successCallback, errorCallback){
    ...
    else if (code !== this.confirmationCode){
        errorCallback(ErrorCodes.InvalidInput);
    }
    else if (this.confirmed){
        errorCallback(ErrorCodes.AlreadyExists);
    }
    else errorCallback();
};
