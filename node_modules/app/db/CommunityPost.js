...
var UserNotificationHelper = require('app/helpers/UserNotificationHelper.js');
...
postSchema.statics.findPostWithId = function(postId, successCallback, errorCallback){
    ...
    this.findOne({postId : postId }).deepPopulate('_comments _user').exec(function(err, post){
        if (err || !post){
...
postSchema.statics.findPosts = function(req, coors, user, successCallback, errorCallback){
    ...
    //
    //}
    var query = this.find(constraints).sort('-dateCreated').limit(numPostsToFetch).deepPopulate('_comments _user');
